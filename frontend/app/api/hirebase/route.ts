...
export async function POST(request: Request) {
  try {
    ...
    if (location.includes(",")) {
      ...
      const countryNames = ["USA", "United States", "Canada", "UK", "United Kingdom", "Australia", "Germany"]
    ...
    try {
      const response = await fetch("https://r87f330d9ps7.share.zrok.io/hirebase/get", {
        method: "POST",
        headers: {
    ...
    const errorMessage =
      error instanceof Error && error.name === "AbortError"
        ...
        : `Failed to fetch data from Hirebase: ${error instanceof Error ? error.message : "Unknown error"}`
